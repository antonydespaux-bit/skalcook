...
export function useAnalysesData({
  ...
  // ── Décomposition par lieu (pour la vue « par lieu ») ─────────────────────
  // Une entrée par lieu présent dans les rows filtrées : totaux N, totaux N-1,
  // et une mini-série d'évolution N/N-1 pour la sparkline. Triée par CA desc.
  const byLieu = useMemo(() => {
    const maybeJours = (rows) => (filterJoursActive ? rows.filter((d) => joursSet.has(d.isoJds)) : rows)
    const groupRows = (rows) => {
      const m = new Map()
      for (const r of rows) {
        if (!m.has(r.lieu_service_id)) m.set(r.lieu_service_id, [])
        m.get(r.lieu_service_id).push(r)
      }
      return m
    }
    const gN = groupRows(filteredRows)
    const gN1 = groupRows(filteredCompareRows)
    const out = [...gN.keys()].map((id) => {
      const rowsN = gN.get(id) || []
      const rowsN1 = gN1.get(id) || []
      let spark = bucketDays(maybeJours(aggregateByDay(rowsN, dateDebut, dateFin)), granularity)
      if (compareRange) {
        const sN1 = bucketDays(maybeJours(aggregateByDay(rowsN1, compareRange.debut, compareRange.fin)), granularity)
        spark = zipCompareBuckets(spark, sN1)
      }
      return {
        id,
        label: lieuxLabels.get(id) || id,
        totals: aggregateTotals(rowsN),
        totalsCompare: comparaison === 'n-1' ? aggregateTotals(rowsN1) : null,
        spark,
      }
    })
    return out.sort((a, b) => b.totals.caTtc - a.totals.caTtc)
  }, [filteredRows, filteredCompareRows, comparaison, compareRange, dateDebut, dateFin, granularity, lieuxLabels, filterJoursActive, joursSet])
  ...
  return {
    ...
    lieuxAffiches,
    // Lignes brutes filtrées (déjà remappées lieu→parent) pour le tableau
    // croisé. On joint N-1 quand la comparaison est active → la dimension
    // « Année » sépare naturellement 2026 / 2025.
    filteredRows, filteredCompareRows, lieuxLabels,
  }
}
