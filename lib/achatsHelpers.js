...
export function enrichLigne(ligne, fournisseurMapping, ingredientsById, ingredientsByNorm, tvaByIngredient = {}) {
  ...
  return {
    ...
    deltaPrix: delta,
    // Conditionnement : si l'ingrédient est reconnu, on affiche le sien (figé) ;
    // sinon on conserve la valeur saisie sur la ligne (défaut 1) — utilisée à la
    // création de l'article.
    conditionnement: ing ? cond : (ligne.conditionnement ?? 1),
    reconnu: !!ingId,
