...
function nbJoursOverride(annee, mois, jds, svcCode, lieuId, joursOverride) {
  // Priorité : (mois, jds, svc, lieu) > (mois, jds, svc, __all__) > calendrier
  const cellMap = joursOverride?.[mois]?.[jds]?.[svcCode]
  if (cellMap) {
    if (lieuId && cellMap[lieuId] != null && cellMap[lieuId] !== '') return Number(cellMap[lieuId])
    if (cellMap.__all__ != null && cellMap.__all__ !== '') return Number(cellMap.__all__)
  }
