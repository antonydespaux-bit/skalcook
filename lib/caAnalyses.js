...
export function dailyBudgetForCell(
  ...
  // 3. Somme des cellules sélectionnées (total MENSUEL, à diviser ensuite)
  let budgetMois = 0
  for (const cell of cellMap.values()) {
    budgetMois +=
      Number(cell.ca_food_cible || 0) +
  ...
  // 4. Conversion total mois → budget journalier. La saisie /budgets écrit
  // un total mensuel par jour-de-semaine (ex: total des 5 dimanches du mois).
  // Pour avoir le budget d'UN dimanche précis, on divise par le nb naturel
  // d'occurrences du jds dans le mois.
  const nbNaturel = countIsoJdsInMonth(annee, mois, isoJds)
  if (nbNaturel === 0) return 0
  const budgetJournalier = budgetMois / nbNaturel

  // 5. Ratio override nb_jours si présent (= proportion de jours ouverts)
  if (overridesByYearMonth) {
    const override = overridesByYearMonth.get(`${annee}_${mois}_${isoJds}`)
    if (override != null) {
      return budgetJournalier * (override / nbNaturel)
    }
  }
  return budgetJournalier
}
...
export function periodBudgetTotal(budgetRowsByYear, debut, fin, isoJdsFilter = null, overridesByYearMonth = null) {
  ...
  for (const d of eachDayIso(debut, fin)) {
    ...
    // map.get(jds) renvoie le TOTAL MENSUEL pour ce jour-de-semaine (ce que
    // stocke la saisie /budgets : la cellule "Dimanche midi" = total des 5
    // dimanches du mois, pas un dimanche unique). On divise par le nb réel
    // d'occurrences naturelles du jds dans le mois pour obtenir le budget
    // d'UNE journée, qu'on additionne ensuite pour chaque jour de la plage.
    const budgetMoisJds = map.get(d.isoJds) || 0
    const nbNaturel = countIsoJdsInMonth(annee, mois, d.isoJds)
    const budgetJournalier = nbNaturel > 0 ? budgetMoisJds / nbNaturel : 0
    total += budgetJournalier * getRatio(annee, mois, d.isoJds)
  }
  return total
}
