...
// Aligne positionnellement les buckets N-1 sur ceux de la période courante.
// La plage N-1 est juste décalée d'un an (même granularité, même nombre de
// points), donc le i-ème point N-1 correspond au i-ème point courant. On colle
// caTotN1 / couvertsN1 sur chaque point courant pour que les charts puissent
// tracer une seconde courbe sans changer l'axe X. Les points sans équivalent
// N-1 (bord de période lors d'un décalage de quantième) restent à null.
export function zipCompareBuckets(buckets, compareBuckets) {
  if (!compareBuckets || compareBuckets.length === 0) return buckets
  return buckets.map((b, i) => {
    const cmp = compareBuckets[i]
    return {
      ...b,
      caTotN1: cmp ? cmp.caTot : null,
      couvertsN1: cmp ? cmp.couverts : null,
    }
  })
}
...
// Fusionne la perf N-1 sur la perf courante par jour-de-semaine (alignement
// direct sur isoJds 1..7, pas d'ambiguïté de quantième ici). Ajoute caN1 / cvN1
// pour permettre un affichage en barres groupées N vs N-1.
export function zipComparePerf(perf, comparePerf) {
  if (!comparePerf || comparePerf.length === 0) return perf
  const byJds = new Map(comparePerf.map((p) => [p.isoJds, p]))
  return perf.map((p) => {
    const cmp = byJds.get(p.isoJds)
    return { ...p, caN1: cmp ? cmp.ca : 0, cvN1: cmp ? cmp.cv : 0 }
  })
}
